...
import { Edit3, Trash2, MapPin, AlertTriangle, Plus, ImageOff } from "lucide-react";
...
const ManageFacilities = ({ownerFacilities = []}) => {
  ...
  // ডিলিট বাটন ক্লিক করলে daisyUI মডাল ট্রিগার হবে
  const handleOpenDeleteModal = (facility) => {
    setSelectedFacility(facility);
    // HTML Dialog ওপেন করার ন্যাটিভ মেথড
  ...
  // কনফার্ম করার পর ডিলিট লজিক
  const handleConfirmDelete = async () => {
    if (!selectedFacility?._id) return;

    // setIsDeleting(true);
    // try {
    //   const res = await deleteFacility(selectedFacility._id);
    //   if (res?.success || res?.deletedCount > 0) {
    //     toast.success(`🎯 "${selectedFacility.name}" has been deleted.`);
    //     handleCloseModal();
    //     window.location.reload(); // পেজ রিফ্রেশ করে লিস্ট আপডেট
    //   } else {
    ...
    return (
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-xs md:text-sm text-gray-400 font-medium mt-1">
            Exclusive Owner Access: Update details or manage slots of your registered arenas.
          </p>
        </div>
        
        {/* daisyUI বোতাম স্টাইল ব্যবহার করে নতুন ফ্যাসিলিটি অ্যাড */}
      ...
      {/* --- ২. মেইন ডাটা টেবিল (daisyUI Table Component) --- */}
      {ownerFacilities.length > 0 ? (
          ...
          <table className="table w-full text-left border-collapse">
            ...
            <thead>
              <tr className="bg-gray-50/80 border-b border-gray-100">
                <th className="font-bold text-gray-500 text-xs uppercase tracking-wider py-4 pl-6">Facility Details</th>
                <th className="font-bold text-gray-500 text-xs uppercase tracking-wider py-4">Type</th>
                <th className="font-bold text-gray-500 text-xs uppercase tracking-wider py-4">Price / Hr</th>
                <th className="font-bold text-gray-500 text-xs uppercase tracking-wider py-4 text-center pr-6">Actions</th>
              </tr>
            </thead>
            ...
            <tbody>
              {ownerFacilities.map((facility) => {
                const isValidImage = typeof facility?.image === "string" && facility.image.trim() !== "";
                
                return (
                  <tr key={facility._id} className="border-b border-gray-50 hover:bg-purple-50/[0.02] transition-colors">
                    {/* কলাম ১: ইমেজ, নাম এবং লোকেশন */}
                    <td className="py-4 pl-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="truncate max-w-[180px] md:max-w-xs">
                          <p className="font-bold text-gray-900 truncate text-sm md:text-base">{facility.name}</p>
                          <div className="flex items-center gap-1 text-gray-400 text-xs mt-0.5 truncate">
                            <MapPin size={12} className="text-rose-500 shrink-0" />
                            <span className="truncate">{facility.location || "Main Center"}</span>
                          </div>
                        </div>
                      </div>
                    </td>

                    {/* কলাম ২: স্পোর্টস টাইপ (daisyUI Badge) */}
                    <td className="py-4">
                    ...
                    {/* কলাম ৪: অ্যাকশন বাটনস (daisyUI Tooltip ও Buttons) */}
                    <td className="py-4 text-center pr-6">
                      <div className="flex items-center justify-center gap-2">
                        {/* আপডেট বাটন */}
                        <div className="tooltip font-bold text-xs" data-tip="Edit Facility">
                          <Link href={`/dashboard/manage-facilities/edit/${facility._id}`}>
                        ...
                        {/* ডিলিট বাটন */}
                        <div className="tooltip font-bold text-xs tooltip-error" data-tip="Delete Facility">
                          <button
        ...
        /* খালি থাকলে এম্পটি স্টেট */
        <div className="text-center py-16 bg-gray-50/50 rounded-[2.5rem] border border-dashed border-gray-200 max-w-md mx-auto px-6">
          <p className="text-gray-400 font-bold text-lg mb-1">No Facilities Registered</p>
          <p className="text-gray-400 text-xs font-medium">You haven't listed any court or arena yet.</p>
        </div>
      )}

      {/* --- ৩. পিওর daisyUI HTML Dialog Modal (Backdrop Blur সহ) --- */}
      <dialog id="delete_confirmation_modal" className="modal modal-bottom sm:modal-middle backdrop-blur-md">
        <div className="modal-box bg-white max-w-md rounded-[2.5rem] border border-gray-100 shadow-2xl p-6">
          ...
          <p className="text-sm text-gray-500 font-medium leading-relaxed">
            Are you absolutely sure you want to permanently delete <strong className="text-gray-900 font-bold">"{selectedFacility?.name}"</strong>? 
            This action cannot be undone and all associated bookings will be automatically dropped.
          </p>
          ...
          <div className="modal-action gap-2 mt-6">
            ...
            >
              {isDeleting ? <span className="loading loading-spinner loading-xs"></span> : "Yes, Delete"}
            </button>
          </div>
        </div>
        
        {/* মডালের বাইরে ক্লিক করলে অটোমেটিক ক্লোজ হওয়ার টেকনিক */}
